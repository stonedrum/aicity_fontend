...
const loadDictTypes = async () => {
  ...
  try {
    const res = await axios.get('http://localhost:8000/dict-types', {
...
const saveDictType = async () => {
  ...
  try {
    if (isEditingDictType.value) {
      await axios.put(`http://localhost:8000/dict-types/${dictTypeForm.value.id}`, dictTypeForm.value, {
        headers: props.authHeaders
      })
    } else {
      await axios.post('http://localhost:8000/dict-types', dictTypeForm.value, {
        headers: props.authHeaders
      })
    }
...
const handleDeleteDictType = async (row) => {
  ...
  try {
    await axios.delete(`http://localhost:8000/dict-types/${row.id}`, {
      headers: props.authHeaders
    })
...
const saveDictData = async () => {
  ...
  try {
    if (isEditingDictData.value) {
      await axios.put(`http://localhost:8000/dict-data/${dictDataForm.value.id}`, dictDataForm.value, {
        headers: props.authHeaders
      })
    } else {
      await axios.post(`http://localhost:8000/dict-data/${selectedDictType.value.id}`, dictDataForm.value, {
        headers: props.authHeaders
      })
    }
    ElMessage.success('保存成功')
...
const handleDeleteDictData = async (row) => {
  ...
  try {
    await axios.delete(`http://localhost:8000/dict-data/${row.id}`, {
      headers: props.authHeaders
    })
    ElMessage.success('删除成功')
    loadDictTypes()
